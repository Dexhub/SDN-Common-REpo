...
int main(int argc, char **argv) {
  ...
  QueueLoggerSampling qs = QueueLoggerSampling(timeFromMs(1000),eventlist);logfile.addLogger(qs);
  ...
  RandomQueue queue(SERVICE, BUFFER, eventlist,&qs,memFromPkt(RANDOM_BUFFER)); queue.setName("Queue"); logfile.writeName(queue);

  Queue pqueue3(SERVICE*2, memFromPkt(FEEDER_BUFFER), eventlist,NULL); pqueue3.setName("PQueue3"); logfile.writeName(pqueue3);
  Queue pqueue4(SERVICE*2, memFromPkt(FEEDER_BUFFER), eventlist,NULL); pqueue4.setName("PQueue4"); logfile.writeName(pqueue4);
  ...
  for (int i=0;i<TCP;i++){
    tcpSrc = new TcpSrc(&logTcp,NULL,eventlist); tcpSrc->setName("Tcp1"); logfile.writeName(*tcpSrc);
    ...
    routeout = new route_t(); routeout->push_back(pqueue);routeout->push_back(&queue); routeout->push_back(&pipe1); routeout->push_back(tcpSnk);
  ...
  // tell it the route
  routeout = new route_t(); routeout->push_back(&pqueue3); routeout->push_back(&queue); routeout->push_back(&pipe1); routeout->push_back(tcpSnk);
  ...
  // tell it the route
  routeout = new route_t(); routeout->push_back(&pqueue4);routeout->push_back(&pipe2);routeout->push_back(&queue); routeout->push_back(&pipe1); routeout->push_back(tcpSnk);
