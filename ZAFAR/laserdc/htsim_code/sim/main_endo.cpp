...
linkspeed_bps SERVICE1 = speedFromPktps(1000);//NUMFLOWS * targetwnd/timeAsSec(RTT1));
...
mem_b BUFFER1=memFromPkt(RANDOM_BUFFER+100);//NUMFLOWS * targetwnd);
...
int main(int argc, char **argv) {
  ...
  logfile.addLogger(sinkLogger);
	...
	for (int i=0;i<TCP_1;i++){
	  ...
	  routeout = new route_t(); routeout->push_back(pqueue);routeout->push_back(&queue1); routeout->push_back(&pipe1); routeout->push_back(tcpSnk);
